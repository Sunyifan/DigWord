...
object RelationByQuery {
		...
		def main(args : Array[String]): Unit ={
			...
			val adTag = Env.sparkContext().textFile("/user/tianxing/adTag/" + Env).filter(_.length > 0).map{
				line : String =>
					val item = line.substring(1, line.length - 1).split(",")
					(item.head.replace("(", ""), item.last.replace(")", ""))
			}.repartition(20)

			val rawUserAction = Env.sparkContext().textFile("/user/tianxing/ua/" + Env).map{
				line : String =>
					val item = line.split(",")
					(item.head.replace("[", ""),
			...
			val rawSeo = Env.sparkContext().textFile("/user/tianxing/seo/" + Env).filter{
			...
			val gelis = Env.sparkContext().textFile("/user/tianxing/geli/all/" + Env).map{line => line.split(",").head}
			...
			result.saveAsTextFile("/user/tianxing/relation/all/" + Env + "-query")
		}
}
